...
class LanguageModel:
    ...
    def train(self, file_loc):
        ...
        token_collection = self._tokenize(sentences)

        self.raw_dict = self.unique_count(token_collection)
        self.processed_dict = self._smooth(self.raw_dict)
        self.corpus_size = self._sum_count(self.processed_dict)

        sum = 0
        for key, value in self.processed_dict.items():
            sum += value

        features = self._featurize(token_collection, self.ngram)
        self.ngram_counts = self.unique_count(features)
        self.processed_ngram_counts = self._smooth(self.ngram_counts)
        self.corpus_size += self.smoothing * len(self.processed_ngram_counts)

        if self.ngram > 1:
            features = self._featurize(token_collection, self.ngram - 1)
            ngram_counts = self.unique_count(features)
            self.n_minus_one_gram = self._smooth(ngram_counts)
        else:
    ...
    def word_count(self):
        return self._sum_count(self.ngram_counts)
        pass
    ...
    def log_prob(self, sentence):
        ...
        for token_index in range(len(tokens)):
            ...
            prob = self._posterior_prob(tokens[token_index], history)
    ...
    def _tokenize(self, sentences):
        collection = []
        for sentence in sentences:
            processed_text = self._preprocess(sentence)
            tokens = processed_text.split(' ')
            collection.extend(tokens)
        return collection

    def _smooth(self, count_list: dict):
        processed_count = {}
    ...
    def _sum_count(self, word_count: dict):
        sum = 0
        for word, count in word_count.items():
            sum += count
        return sum

    def _posterior_prob(self, word, history=None):
        # print(P(word, '|', history))
    ...
    def _featurize(self, tokens, gram):
        features = []
        for token_index in range(len(tokens) - 1):
            if self.raw_dict[tokens[token_index]] <= 1:
